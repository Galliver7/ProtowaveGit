...
#if defined(NDEBUG)
# error "Protowave cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xa74e79966bec7f6735b7555909e2bf9b61d43f17761d519bb3c9aa70e6a72f25");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Protowave: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Protowave Signed Message:\n";
...
{
    // Protowave: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 840000 blocks, which will occur approximately every 5month
    nSubsidy >>= (nHeight / 168750); // Protowave: 168750 blocks in ~5 month

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Protowave: 1 days
static const int64 nTargetSpacing = 1.25 * 60; // Protowave: 1.25 minutes(75sec)
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Protowave: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Protowave: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Protowave: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "2014/7/2 Syuudantekijieikenn ga kakugikettei";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x9799d888bfbe4b557ebfcb2e59bd78d2ac629088fff41538eb4bba618c37b84a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0x50, 0x52, 0x54, 0x4f }; // Protowave: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ProtowaveMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ProtowaveMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ProtowaveMiner(CWallet *pwallet)
{
    printf("ProtowaveMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("protowave-miner");
    ...
    try { loop {
        ...
        printf("Running ProtowaveMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ProtowaveMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ProtowaveMiner, pwallet));
}
